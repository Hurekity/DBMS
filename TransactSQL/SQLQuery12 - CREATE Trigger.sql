...
BEGIN
IF 
(SELECT [group] FROM Schedule WHERE (SELECT MAX(Schedule.number_of_lesson) FROM Schedule WHERE spent=1) NOT IN (SELECT * FROM CompleteDisciplines))
...
BEGIN 
INSERT INTO CompleteDisciplines (discipline, [group])
VALUES 
(
